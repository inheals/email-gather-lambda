    ...
    dueArray.map(app=>{
        ...
        if(app.length){
          app.map(bill=>{
            ...
            console.log('yes',formatMonth(bill.bill_due_month) == (new Date().getMonth()+1).toString())
            if(bill.bill_due_month && formatMonth(bill.bill_due_month) == (new Date().getMonth()+1).toString()){
    ...
    paidArray.map(app=>{
        if(app.length){
          ...
          app.map(bill=>{
            ...
            if(bill.bill_paid_month && Object.keys(bill).length && formatMonth(bill.bill_due_month) == (new Date().getMonth()+1).toString()){
              billDetails[bill.appName] = {... billDetails[bill.appName], ...bill}
